...
{/*PhpDoc: classes
name: UnitTest
title: class UnitTest - classe regroupant les 2 mécanismes de test
methods:
doc: |
...
class UnitTest {
  ...
  static function print_header(string $file) {
    if (self::$first) {
  ...
  /*PhpDoc: methods
  name: class
  title: "static function class(string $nameSpace, string $currentfile, string $class): void - effectue les tests unitaires de la classe fournie"
  doc: |
    ...
    Si le paramètre class est défini mais que method ne l'est pas alors la fonction détermine la liste des méthodes à tester
    qui sont celles pour lesquelles une méthodes de test a été définie avec un nom commencant par test_ puis le nom de la méthode ;
    ...
    Par défaut les méthodes héritées ne sont pas proposées pour le test. Pour tester une méthode redéfinie dans une classe fille,
  ...
  */
  ...
  /*PhpDoc: methods
  name: function
  title: "static function function(string $currentfile, string $functionName, callable $test_function): void - effectue le test unitaire de la fonction fournie"
  doc: |
    $currentfile doit être __FILE__ au moment de l'appel afin de correspondre au chemin du fichier du source 
...
class CATester {
  static function methodeStatiqueATester() { echo "méthode statique à tester<br>\n"; }
  static function test_methodeStatiqueATester() { self::methodeStatiqueATester(); }
  
  function methodeNonStatiqueATester() { echo "méthode non statique à tester<br>\n"; }
  static function test_methodeNonStatiqueATester() { (new CATester)->methodeNonStatiqueATester(); }
  
  function methodA() { echo "CATester::methodeA<br>\n"; }
  static function test_methodA() { (new self)->methodA(); }
};

UnitTest::class(__NAMESPACE__, __FILE__, 'CATester');

class CFille extends CATester {
  function methodA() { echo "CFille::methodeA<br>\n"; }
  static function test_CFille_methodA() { (new self)->methodA(); }
  
  static function test_methodFille() {}
};

UnitTest::class(__NAMESPACE__, __FILE__, 'CFille');

function fonction_a_tester() { echo "fonction à tester<br>\n"; }
